...
export function TextInline({ value, onChange, placeholder = '—', type = 'text', dense = false }: {
  ...
  return (
    <input ref={ref} value={v} type={type} placeholder={placeholder}
      ...
      style={{ width: '100%', height: dense ? 30 : 32, padding: '0 6px', margin: '0 -6px', border: '1px solid transparent', borderRadius: 6, background: 'transparent', fontSize: dense ? 12.5 : 13, color: v ? 'var(--fe-text)' : 'var(--fe-text-faint)', outline: 'none', transition: 'border-color var(--fe-dur-fast), background var(--fe-dur-fast)' }}
