 ...
 * Emite HTML via onChange. O conteúdo inicial é semeado uma vez para não
 * resetar o cursor a cada digitação; o auto-save (debounce) fica a cargo do pai.
 */
export function RichTextEditor({
  ...
  // Semeia o HTML inicial apenas na montagem.
  useEffect(() => {
  ...
  function aplicar(cmd: string, val?: string) {
    ref.current?.focus()
    exec(cmd, val)
    emitir()
  }
  ...
  return (
    <div style={{ border: `1px solid ${focado ? 'var(--fe-accent)' : 'var(--fe-border-soft)'}`, borderRadius: 'var(--fe-radius-lg)', overflow: 'hidden', transition: 'border-color var(--fe-dur-fast)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 2, padding: '5px 6px', borderBottom: '1px solid var(--fe-divider)', background: 'var(--fe-warm-white)', flexWrap: 'wrap' }}>
        <button type="button" title="Título" style={tbBtn()} onMouseDown={(e) => e.preventDefault()} onClick={() => aplicar('formatBlock', 'H2')}>
          <span style={{ fontWeight: 700, fontSize: 13 }}>H1</span>
        </button>
        <button type="button" title="Subtítulo" style={tbBtn()} onMouseDown={(e) => e.preventDefault()} onClick={() => aplicar('formatBlock', 'H3')}>
          <span style={{ fontWeight: 700, fontSize: 12 }}>H2</span>
        </button>
        <button type="button" title="Texto normal" style={tbBtn()} onMouseDown={(e) => e.preventDefault()} onClick={() => aplicar('formatBlock', 'P')}>
          <span style={{ fontWeight: 500, fontSize: 12 }}>P</span>
        </button>
        <Sep />
        <button type="button" title="Negrito" style={tbBtn()} onMouseDown={(e) => e.preventDefault()} onClick={() => aplicar('bold')}>
          <b style={{ fontSize: 14 }}>B</b>
        </button>
        <button type="button" title="Itálico" style={tbBtn()} onMouseDown={(e) => e.preventDefault()} onClick={() => aplicar('italic')}>
          <i style={{ fontSize: 14, fontFamily: 'Georgia, serif' }}>I</i>
        </button>
        <button type="button" title="Sublinhado" style={tbBtn()} onMouseDown={(e) => e.preventDefault()} onClick={() => aplicar('underline')}>
          <u style={{ fontSize: 13 }}>U</u>
        </button>
        <button type="button" title="Tachado" style={tbBtn()} onMouseDown={(e) => e.preventDefault()} onClick={() => aplicar('strikeThrough')}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 6, padding: 2 }}>
              {CORES.map((c) => (
                <button key={c} type="button" onMouseDown={(e) => e.preventDefault()}
                  onClick={() => { aplicar('foreColor', c); close() }}
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div
          ...
          onBlur={() => { setFocado(false); emitir() }}
          className="fe-richtext"
          style={{ minHeight, padding: '11px 14px', fontSize: 13.5, lineHeight: 1.6, color: 'var(--fe-text)', outline: 'none' }}
        />
      </div>
    </div>
  )
...
function Sep() {
  return <span style={{ width: 1, height: 18, background: 'var(--fe-border-soft)', margin: '0 3px', flexShrink: 0 }} />
}

/** Render somente-leitura do HTML salvo. */
