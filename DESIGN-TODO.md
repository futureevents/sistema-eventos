...
| Parte 5 — `/clarify` (acessibilidade) | ✅ **Código pronto** — teclado nas linhas/grupos, anel de foco `:focus-visible` global e `aria-label` nos ícones; `tsc` limpo. ⚠️ Falta conferir no preview (porta 3000 ocupada por outro dev server) |
...
### ✅ Acessibilidade → `/clarify` — CÓDIGO PRONTO (preview pendente)
Arquivos: `DataList.tsx`, `kit.tsx`, `inline.tsx`, `globals.css`
- [x] Linhas e cabeçalhos de grupo agora têm `role="button"` + `tabIndex={0}` + `onKeyDown` (Enter/Espaço) — antes `<div onClick>` sem teclado; o guard `e.target === e.currentTarget` evita disparar quando o foco está num controle interno. Grupo também ganhou `aria-expanded`.
- [x] Anel de foco visível restaurado de forma global em `globals.css`: `:focus-visible` com `outline 2px var(--fe-accent)` + `!important` (única forma de vencer os `outline:none` inline dos inputs/células). Linhas/grupos usam offset negativo (anel interno) p/ não serem cortados pelo `overflow:hidden` do card.
- [x] `aria-label` adicionado nos botões só-ícone: status (boinha), menu de linha (⋯), navegação do calendário (mês ant./próx.), expandir/mais/fechar/ocultar-campo do painel, e convidar/compartilhar/mais do breadcrumb.
- [ ] ⚠️ Conferir no preview (Tab numa lista p/ ver o anel) — não foi possível subir servidor isolado: Next 16 só permite um `next dev` por projeto e a porta 3000 está ocupada por outro chat.
...
5. ✅ ~~`/clarify` — acessibilidade~~ — **código pronto** (preview pendente)
6. `/quieter` → `/delight` → `/polish` — acabamento ← **próximo**
